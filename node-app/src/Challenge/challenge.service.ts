...
export class ChallengeService {
  ...
  async update(updatedChallenge: ChallengeData): Promise<Challenge | null> {
    try {
      const existingChallenge = await this.challengeModel.findById(
        updatedChallenge.id,
      );
  ...
  async updateDailyChallenges() {
    ...
    const dailyChallenges = await this.challengeModel
      .find({
        endDate: {
          $gte: moment(new Date()).add(-1, 'day').toDate(),
          $lte: new Date(),
        },
  ...
  async updateWeeklyChallenges() {
    ...
    const weeklyChallenges = await this.challengeModel
      .find({
        endDate: {
          $gte: moment(new Date()).add(-1, 'week').toDate(),
          $lte: new Date(),
        },
  ...
  async updateMonthlyChallenges() {
    ...
    const monthlyChallenges = await this.challengeModel
      .find({
        endDate: {
          $gte: moment(new Date()).add(-1, 'month').toDate(),
          $lte: new Date(),
        },
        periodicity: Periodicity.WEEKLY,
      })
