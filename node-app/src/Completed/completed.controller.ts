...
export class CompletedController {
  ...
  @ApiResponse({ status: 200, description: 'Successfully retrieved all completed challenges.', type: [Completed] })
  ...
  @ApiBody({ type: CreateCompletedDto, description: 'Data for the new completed challenge record' })
  @ApiResponse({ status: 201, description: 'Completed challenge record successfully created.', type: Completed })
  ...
  @ApiParam({ name: 'userId', type: String, required: true, description: 'User ID to retrieve completed challenges for' })
  @ApiResponse({ status: 200, description: 'Successfully retrieved completed challenges for the user.', type: [Completed] })
  ...
  @ApiOperation({ summary: 'Get completed challenges by user ID and challenge ID' })
  @ApiQuery({ name: 'userId', type: String, required: true, description: 'User ID' })
  @ApiQuery({ name: 'challengeId', type: String, required: true, description: 'Challenge ID' })
  @ApiResponse({ status: 200, description: 'Successfully retrieved the completed challenges based on user ID and challenge ID.', type: [Completed] })
  async getCompletedByUserIdByChallengeId(
    @Query('userId') userId: Types.ObjectId,
    @Query('challengeId') challengeId: Types.ObjectId
  ): Promise<Completed[]> {
    return this.completedService.getCompletedByUserIdByChallengeId(userId, challengeId);
  }
  ...
  @ApiQuery({ name: 'userId', type: String, required: true, description: 'User ID' })
  @ApiQuery({ name: 'challengeId', type: String, required: true, description: 'Challenge ID' })
  @ApiResponse({ status: 204, description: 'Completed challenge successfully deleted' })
